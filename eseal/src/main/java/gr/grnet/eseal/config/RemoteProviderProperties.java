...
/**
 * RemoteProviderProperties holds the configuration properties regarding the interaction of the API
 * with the Provider's rest api for signing and timestamping documents
 */
@Setter
@Getter
@NoArgsConstructor
...
public class RemoteProviderProperties {
  ...
  /** Remote provider's rest api endpoint this value will map to eseal.remote.provider.endpoint */
  private String endpoint;

  /** Retry requests on the remove provider if an error occurs */
  ...
  /**
   * Try to verify the remote API certs using a client truststore, otherwise use a TRUST_ALL
   * strategy
   */
